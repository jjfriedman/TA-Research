...
QueryInstitution_ROR_ID = "https://ror.org/010x8gc63" #ROR ID is University of Saskatchewan.
...
QueryPublisher = "p4310320595" #Publisher ID is Wiley
...
Institutional_Works <- oa_fetch(
  ...
  primary_location.source.publisher_lineage = QueryPublisher, #limit to publisher
  verbose = TRUE,
...
QueryStructure <- c("Entity", "Institution(s) ROR(s)", "Type", "Source Type", "Start Date", "End Date", "Timestamp", "Warnings") #Query labels
QueryValues <- c(QueryEntity, QueryInstitution_ROR_ID, QueryType, QuerySourceType, QueryStartDate, QueryEndDate, CurrentTime, QueryWarnings) #Query values
