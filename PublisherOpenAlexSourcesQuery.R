...
QueryPublisher = "p4324035611" #Enter publisher ID here. Default is Érudit Consortium

#This code uses the query variables above to call the OpenAlex API
Publisher_Sources <- oa_fetch(
  entity = QueryEntity, #query type
  host_organization = QueryPublisher,
  verbose = TRUE,
...
QueryStructure <- c("Entity","Publisher(s)", "Timestamp", "Warnings") #Query labels
QueryValues <- c(QueryEntity,QueryPublisher, CurrentTime, QueryWarnings) #Query values
...
FilteredSources <- Publisher_Sources %>% select(-any_of(FilterColumns)) #Removes columns indicated above
FilteredSourcesOnly <- FilteredSources %>% select(-any_of(c("issn","apc_prices"))) #Removes nested data authorships/apc for a clean works list for export
OASourcesOnly <- filter(FilteredSourcesOnly,is_oa == TRUE) #Generate OA sources list

#Generate All ISSN list
AllISSNs <- unnest(FilteredSources, issn, names_sep = "_") #Unnests ISSNs so that all ISSNs are listed
AllISSNsOnly <- AllISSNs %>% select(!"apc_prices") #Removes nested data affiliations and apc for a clean authors list for export

#Generate All ISSNs All APCs list
AllAPCsAllISSNs <- unnest(AllISSNs, apc_prices, names_sep = "_") #Unnests affiliations so that all affiliations are listed
...
ListofWorksheets <- list("Guide" = GuideSheet,
                         "Query" = Query,
                         "Sources" = FilteredSourcesOnly, 
                         "OASources" = OASourcesOnly,
                         "AllISSNs" = AllISSNsOnly,
                         "AllISSNsAndAPCs" = AllAPCsAllISSNs
                          )


#Export

OutputFile <- paste(CurrentTime, "OpenAlex_TA", "Sources", ".xlsx", sep = "_") #Build output file name using query values
